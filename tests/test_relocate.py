...
def test_straight_quotes_and_hyphen_match_curly_quotes_and_en_dash(document: Document) -> None:
    ...
    assert match is not None
    assert match.method == "normalised"
    assert round_trip(document, match) == (
        "recorded over 2019–2021 on the “gold” subset"
    )
...
def test_fuzzy_span_is_widened_to_whole_words(document: Document) -> None:
    match = locate(document, "blating the attention block costs 3.4 ponts")

    assert match is not None
    assert match.method == "fuzzy"
    assert round_trip(document, match) == "Ablating the attention block costs 3.4 points"


def test_fuzzy_span_does_not_reach_past_the_quote(document: Document) -> None:
    match = locate(document, "segment cells in microscopy images.\nWe introdue")
