...
| | |
|---|---|
| **Schema** | [`packs/scierc.yaml`](../../packs/scierc.yaml): six mention types, seven relations. A local, versioned profile of the SciERC annotation scheme, with `dct:source` on every term and `atlas:` identities of our own — the scheme publishes no OWL IRIs and this pack does not invent any. A partial TBox and RBox: types, a relation signature, no axioms. |
| **Reasoner** | None. The pack is checked for shape, domain and range when a node or a link is written, and nothing is inferred. |
...
| **Evolution** | Editing the pack and running again. There is no candidate pool, no promotion gate and no evidence for a change: a concept the corpus keeps using that the profile has no word for is simply not marked up. |
...
2. **`extract_llm`** asks the model, once per page, for typed statements with a
   verbatim quote each. The types on offer are read out of the loaded pack, so this
   step names no vocabulary.
3. **`relocate`** finds each quote in the frozen text and mints a node from the span it
   ...
   approximately.
4. **`validate`** drops the nodes the pack refuses and keeps the violations.
5. **`relate_llm`** asks, once per page that holds at least two nodes, which of them the
   ...
   and the relations the pack declares.
6. **`relate`** places each claimed relation's quote and mints a `Link` — but only if
   the pack allows that predicate between those two types. A forbidden pairing is
   reported as a violation with its case, not counted away.
7. **`assert`** writes every node and link into the store as one assertion each,
   attributed to the run.
8. **`index_nodes`** builds the inverted index beside the store.
...
2. **`graph_expand`** walks out from those seeds — two hops, at most forty nodes,
   every relation the pack declares — and returns a `Bundle`.
3. **`graph_answer`** shows the model the entries, the relations in the direction they
...
result, a condition or a computation — the five that `packs/science_core.yaml` adds
...
changes when somebody edits `packs/scierc.yaml`, which changes `Schema.version` and
...
- **`compare` is not "better than".** The scheme is explicit about this and so is the
  pack; an answer that turns a comparison into a ranking is the failure to watch for.
...
| Tests | `tests/test_relate.py`, `tests/test_graph_expand.py`, `tests/test_graph_answer.py`, `tests/test_catalogue.py` |
