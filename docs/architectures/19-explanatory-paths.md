...
| | |
|---|---|
| **Schema** | `science_core` + `scierc`. Relations carry the meaning a path is made of. |
| **Reasoner** | None. Enumeration and pruning. |
...
## 4. The two rules that decide whether pruning is safe

### 4.1 Redundancy is judged by content, not by text
...
### 4.2 A counter-path is never pruned
...
- **Losing the rare counter-path** is the risk, and §4.2 is the answer. It is tested by
