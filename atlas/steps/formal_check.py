...
CHECKS = ("hierarchy", "disjointness", "relations", "inverses")
"""The four groups of checks, named so that a budget can say which of them it skipped."""
...
def formal_check(state: State, options: FormalCheckOptions) -> State:
    ...
    report = inspect(state["schema"], budget=options.budget)
...
def inspect(schema: Schema, budget: int = 2000) -> Report:
    """Everything wrong with a pack, in four groups, with what was skipped for budget."""
    ...
    return Report(problems=tuple(problems), checked=CHECKS)
...
def _relations(schema: Schema) -> list[Problem]:
    ...
    for predicate in schema.predicates:
        if schema.find_type(predicate.domain) is None:
            found.append(Problem(kind="unknown-domain", term=predicate.name,
                                 detail=f"domain {predicate.domain!r} is not a type of this pack"))
        if schema.find_type(predicate.range) is None:
            found.append(Problem(kind="unknown-range", term=predicate.name,
                                 detail=f"range {predicate.range!r} is not a type of this pack"))
        found += [
