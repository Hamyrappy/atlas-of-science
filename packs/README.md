...
`atlas.ontology.load(*paths)` merges packs in the order given and versions the result by the sha256 of their bytes, which is the version every node and link then carries.

A type declares `name`, `iri`, an optional `parent` whose fields it inherits, its own `fields` (a bare name, or a mapping with `datatype` and `iri`), optional `mappings` to public vocabularies, and a `description`; a predicate declares `name`, `iri`, `domain`, `range` and a `description`.
